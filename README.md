...
เราจะทดลองยิง load test ด้วย k6 ที่ `https://localhost:8000/products` โดยเป็นการดึงข้อมูลสินค้า 30 รายการ เปรียบเทียบ 4 แบบ ทั้งไม่ใช้ cache เลย และใช้ cache ที่ layer ต่าง ๆ

![รูป Requests per Second](img/rediscmp.png)

| แบบทดสอบ | Cache ที่ layer | Requests/sec (peak) |
|---|---|---|
| No cache | — (query DB ทุก request) | ~800 |
| Cache at Repositories | Repositories — cache ผล query จาก DB | ~2,400 |
| Cache at Services | Services — cache ผลหลังผ่าน business logic | ~2,600 |
| Cache at Handlers | Handlers — cache JSON response ทั้งก้อน | ~3,000 |

## โครงสร้างโปรเจกต์

```
├── main.go              # ประกอบ dependency + Fiber server
├── .env.example         # ตัวอย่าง config (copy เป็น .env แล้วแก้ค่า)
├── Makefile             # คำสั่งลัด (up / run / test / ...)
├── handlers/            # HTTP handler layer (ปกติ / cache)
├── services/            # Business logic layer (ปกติ / cache)
├── repositories/        # Data access layer (ปกติ / cache)
├── docker-compose.yml   # PostgreSQL, Redis, k6, InfluxDB, Grafana
├── config/
│   ├── config.go        # โหลด config จาก environment variables
...
ก่อนรันครั้งแรก ให้ copy [.env.example](.env.example) เป็น `.env` แล้วแก้ค่าตามเครื่องตัวเอง (เช่น `DB_PASSWORD`):

```bash
cp .env.example .env
```

ทั้ง Go server และ docker compose จะอ่านค่าจากไฟล์ `.env` เดียวกัน (ไฟล์นี้อยู่ใน .gitignore จะไม่ถูก push ขึ้น git)
...
หลังจากใช้คำสั่ง `make up` เปิด `http://localhost:3000` บน เบราว์เซอร์ จะเจอหน้า Grafana (ที่สามารถเข้าได้เลยไม่ต้อง login เพราะเราตั้งค่า anonymous Admin ใน [docker-compose.yml](docker-compose.yml) ไว้)
...
ไปที่ **Dashboards → New → Import** ใส่ dashboard ID **2587** (k6 Load Testing Results) กด **Load** แล้วเลือก data source เป็น InfluxDB ที่เพิ่งสร้าง จากนั้นกด **Import** จะเห็น dashboard ของ grafana
...
โค้ดจัดโครงสร้างตามแนว hexagonal architecture — แต่ละ layer (repositories / services / handlers) มี interface กลาง จึงสลับ implementation ระหว่างเวอร์ชันปกติกับเวอร์ชัน Redis ได้ที่จุดประกอบ dependency ใน [main.go](main.go) โดย comment/uncomment บรรทัดที่ต้องการ:
